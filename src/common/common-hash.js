...
var factory = function (Util, Crypto, Keys) {
    ...
    Hash.hashChannelList = function (list) {
        return Util.encodeBase64(Crypto.Random.createHash(Util
    ...
    Hash.getSignPublicFromPrivate = function (edPrivateSafeStr) {
        ...
        var keyPair = Crypto.Random.signKeyPairFromSecretKey(privateKey);
    ...
    Hash.getCurvePublicFromPrivate = function (curvePrivateSafeStr) {
        ...
        var keyPair = Crypto.Random.boxKeyPairFromSecretKey(privateKey);
        return Util.encodeBase64(keyPair.publicKey);
    };
    ...
    Hash.createChannelId = function (ephemeral) {
        var id = uint8ArrayToHex(Crypto.Random.bytes(ephemeral? 17: 16));
    ...
    */
    Hash.getBoxPublicFromSecret = function (priv) {
        ...
        var pair = Crypto.Random.boxKeyPairFromSecretKey(u8_priv);
...
*/
    ...
    Hash.getFormData = function (secret, hash, password) {
        ...
        if (!secondary) { return; }
        var curvePair = Crypto.Random.boxKeyPairFromSecretKey(Util.decodeUTF8(secondary).slice(0,32));
        var ret = {};
