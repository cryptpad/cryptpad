...
const completeUpload = function (data, cb) {
    ...
    Env.blobStore[method](safeKey, arg, function (err, id) {
        ...
        var maxExpire = new Date().setMonth(new Date().getMonth() + 100);
        if (data.expire && typeof(data.expire) === 'number' && data.expire < maxExpire) {
            Env.tasks.write(data.expire, "EXPIRE_BLOB", [ id ], function (err) {
                if (err) {
