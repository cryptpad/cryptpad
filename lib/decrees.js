...
commands.ENABLE_LOG_RPC = (Env, args) => {
    if (!Array.isArray(args) || args.length !== 1 || typeof(args[0]) !== "boolean") {
        throw new Error("INVALID_ARGS");
    }
    let state = Boolean(args[0]);
    if (state && Env.enableLogPin) { return false; }
    if (!state && !Env.enableLogPin) { return false; }

    if (!state) {
        try {
            let logger = Env.enableLogPin;
            delete Env.enableLogPin;
            if (logger?.shutdown) { logger.shutdown(); }
    ...
    c.logLevel = 'info';
    Env.enableLogPin = true;
    Logger.create(c, logger => {
        if (!Env.enableLogPin) { return void logger.shutdown(); }
        Env.enableLogPin = logger;
    });
    return true;
};
