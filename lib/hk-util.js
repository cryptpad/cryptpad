...
const getMetadata = HK.getMetadata = function (Env, channelName, _cb) {
    ...
    MetaRPC.getMetadataRaw(Env, channelName, function (err, metadata) {
        ...
        // If metadata is not in cache it's a new pad open
        var cmetadata = Env.metadata_cache[channelName];
        if (cmetadata && typeof(cmetadata) === 'object') {
          console.log("DEBUG: getMetadata no cache ", channelName);
          Env.Stats.addOpenPad(Env);
        }
        // cache it
...
const handleFirstMessage = function (Env, channelName, metadata) {
    // Let's call the stats to count the new channel
    Env.Stats.addNewPad(Env);
    Env.Stats.addOpenPad(Env);
