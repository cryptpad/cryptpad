...
const checkExpired = function (Env, Server, channel) {
    ...
    store.closeChannel(channel, function () {
        Server.channelBroadcast(channel, { // XXX reuse this format in onChannelMessage
            error: 'EEXPIRED',
            channel: channel
        }, Env.id);
...
const handleGetHistoryRange = function (Env, Server, seq, userId, parsed) {
    ...
    if (typeof(desiredMessages) !== 'number' && typeof(desiredCheckpoint) !== 'number' && !untilHash) {
        // XXX respond with txid (swap this with the following block)
        // otherwise this will guard against multiple recipients receiving stuff
        // https://github.com/xwiki-labs/cryptpad/blob/staging/www/common/outer/async-store.js#L2090
    ...
    Server.send(userId, [seq, 'ACK']); // XXX move this into the following async flow
    Env.getOlderHistory(channelName, oldestKnownHash, untilHash, desiredMessages, desiredCheckpoint, function (err, toSend) {
        if (err && err.code !== 'ENOENT') {
            Env.Log.error("HK_GET_OLDER_HISTORY", err); // XXX
            // make sure whatever error code we send will be supported
            // return void Server.send(userId, [seq, 'ERROR', 'UNSPECIFIED_COUNT', HISTORY_KEEPER_ID]);
            // XXX UI can handle it, worker does not --Yann
        }
...
HK.onDirectMessage = function (Env, Server, seq, userId, json) {
    ...
    var channelName = parsed[1];
    nThen(function (w) {
        getMetadata(Env, channelName, w(function (err, metadata) { // XXX streamClose
            if (err) {
                Log.error("HK_DIRECT_MESSAGE_METADATA", {
                    channel: channelName,
                    error: err.message || err,
                });
                // XXX poor reasoning in this comment
                // stream errors?
                // if we can't load metadata
...
*/
            ...
            w.abort();
            return void Server.send(userId, [ // XXX use this error format above
                seq,
                'ERROR',
                'ERESTRICTED',
                HISTORY_KEEPER_ID
