...
var expireBlob = function (env, task, cb) {
    var Log = env.log;
    var args = task.slice(2);

    Log.info('ARCHIVAL_SCHEDULED_BLOB_EXPIRATION', { task: task });
    env.blobStore.archive.blob(args[0], 'EXPIRED', function (err) {
        if (err) {
            Log.error('ARCHIVE_SCHEDULED_BLOB_EXPIRATION_ERROR', { task: task, error: err });
        }
        cb();
    });
};
...
var run = Tasks.run = function (env, path, cb) {
    ...
    nThen(function (w) {
        ...
        switch (command) {
            ...
            case 'EXPIRE_BLOB':               
                return void expireBlob(env, task, w());
            default:
