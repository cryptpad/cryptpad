    ...
    */
    ...
    nThen(function (w) {
        ...
        // OLDER METHODS
            ...
            message: function (channelName, content, cb) { // XXX backup
        ...
        // NEWER IMPLEMENTATIONS OF THE SAME THING
            // write a new message to a log
            messageBin: (channelName, content, cb) => { // XXX backup
                if (!isValidChannelId(channelName)) { return void cb(new Error('EINVAL')); }
                schedule.ordered(channelName, function (next) {
...
// either method introduces significant complexity.
        ...
        // METHODS for deleting data
            ...
            removeChannel: function (channelName, cb) { // XXX backup
                if (!isValidChannelId(channelName)) { return void cb(new Error('EINVAL')); }
...
// resulting in a junk/unusable document
            ...
            removeArchivedChannel: function (channelName, cb) { // XXX backup
                if (!isValidChannelId(channelName)) { return void cb(new Error('EINVAL')); }
                schedule.ordered(channelName, function (next) {
            ...
            clearChannel: function (channelName, cb) { // XXX backup
                if (!isValidChannelId(channelName)) { return void cb(new Error('EINVAL')); }
                schedule.ordered(channelName, function (next) {
                    clearChannel(env, channelName, Util.both(cb, next));
                });
            },
            trimChannel: function (channelName, hash, cb) { // XXX backup
                if (!isValidChannelId(channelName)) { return void cb(new Error('EINVAL')); }
...
// since it's impossible to win the race every time let's just make this 'unordered'
            ...
            archiveChannel: function (channelName, cb) { // XXX backup
                if (!isValidChannelId(channelName)) { return void cb(new Error('EINVAL')); }
...
// so I'm calling this 'unordered' again
            ...
            restoreArchivedChannel: function (channelName, cb) { // XXX backup
                if (!isValidChannelId(channelName)) { return void cb(new Error('EINVAL')); }
...
// you can always remove them without data loss
            clearOffset: function (channelName, _cb) { // XXX backup
                var cb = Util.once(Util.mkAsync(_cb));
                if (!isValidChannelId(channelName)) { return void cb(new Error('EINVAL')); }
                clearOffset(env, channelName, cb);
            },
            writeOffset: function (channelName, data, _cb) { // XXX backup
                var cb = Util.once(Util.mkAsync(_cb));
                if (!isValidChannelId(channelName)) { return void cb(new Error('EINVAL')); }
...
// same logic as 'readDedicatedMetadata
            ...
            writeMetadata: function (channelName, data, cb) { // XXX backup
                if (!isValidChannelId(channelName)) { return void cb(new Error('EINVAL')); }
...
// In any case, it will be re-opened if anyone tries to write to it.
            ...
            // write to a log file
            log: function (channelName, content, cb) { // XXX backup
