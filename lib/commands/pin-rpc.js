...
var getChannelList = Pinning.getChannelList = function (Env, safeKey, _cb) {
    ...
    loadUserPins(Env, safeKey, function (err, pins) {
        if (err) {
            return void cb(err);
        }
        cb(void 0, truthyKeys(pins));
    });
};

Pinning.getTotalSize = function (Env, safeKey, _cb) {
    var cb = Util.once(Util.mkAsync(_cb));
    ...
    Env.batchTotalSize(batchKey, cb, function (done) {
        ...
        nThen(function (waitFor) {
            ...
            getChannelList(Env, safeKey, waitFor(function (err, _channels) {
                if (err) {
                    waitFor.abort();
                    return void cb(err);
                }
            ...
            if (limit && Array.isArray(limit.users) && limit.users.length > 1) {
                limit.users.forEach(function (key) {
                    ...
                    getChannelList(Env, key, waitFor(function (err, _channels) {
                        if (err) {
                            waitFor.abort();
                            return void cb(err);
                        }
...
var getHash = Pinning.getHash = function (Env, safeKey, cb) {
    getChannelList(Env, safeKey, function (err, channels) {
        if (err) {
            return void cb(err);
        }
...
Pinning.pinChannel = function (Env, safeKey, channels, cb) {
    ...
    getChannelList(Env, safeKey, function (err, pinned) {
        if (err) { return void cb(err); }
