...
Block.writeLoginBlock = function (Env, safeKey, msg, _cb) { // FIXME BLOCKS // XXX backup
    var cb = Util.once(Util.mkAsync(_cb));
    //console.log(msg);
    var publicKey = msg[0];
    var signature = msg[1];
    ...
    nThen(function (w) {
        ...
        Fs.writeFile(path, Buffer.from(validatedBlock), { encoding: "binary", }, function (err) { // XXX backup
...
*/
Block.removeLoginBlock = function (Env, safeKey, msg, cb) { // FIXME BLOCKS // XXX backup
    var publicKey = msg[0];
    var signature = msg[1];
    ...
    nThen(function (w) {
        ...
        validateLoginBlock(Env, publicKey, signature, block, function (e /*::, validatedBlock */) {
            ...
            // FIXME COLDSTORAGE
            Fs.unlink(path, function (err) { // XXX backup
