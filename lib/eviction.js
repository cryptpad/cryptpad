...
var BlockStore = require("../lib/storage/block");
    ...
    */
    ...
    // iterate over all accounts
        ...
        var handler = function (content, id, next) {
            ...
            if (content.block) {
                if (Env.DRY_RUN) {
                    return Log.info('EVICT_INACTIVE_ACCOUNT_BLOCK', content.block, todo);
                }
                BlockStore.archive(Env, content.block, 'INACTIVE', err => {
                    if (err) {
                        return Log.error('EVICT_INACTIVE_ACCOUNT_BLOCK', err, todo);
                    }
                    Log.info('EVICT_INACTIVE_ACCOUNT_BLOCK', content.block, todo);
                });
                return;
            }
            todo();
