(function (global, factory) {
	...
	(global = global || self, global.view = factory());
	...
	function persistCSS(items) {
	  return items.map(item => {
	    if (item.type === 'stylesheet') {
	      return (0, html$1.wrapHtml)('link', null, Object.assign({
	        rel: 'stylesheet'
	      }, item.data));
	    }
	...
	function initialize(Markmap, options) {
	  Markmap.transformHtml.tap((mm, nodes) => {
	    ...
	    const langs = (0, util.flatMap)(nodes, node => (0, util.arrayFrom)(node.querySelectorAll('code[class*=language-]'))).map(code => {
	...
	class Markmap {
	  constructor(svg, opts) {
	    ...
	    this.options = Object.assign({
	      ...
	      autoFit: false,
	      fitRatio: 0.95,
	      color: (colorFn => node => colorFn(node.p.i))(d3$1.scaleOrdinal(d3$1.schemeCategory10)),
	      paddingX: 8
	    }, opts);
	    this.state = {
...
`;
	  ...
	  initializeData(node) {
	    ...
	    container.className = (0, util.addClass)(container.className, `${id}-fo`, containerClass);
...
`;
	    ...
	    (0, util.walkTree)(node, (item, next) => {
	      var _item$c;
	    ...
	    const nodes = (0, util.arrayFrom)(container.childNodes);
	    this.constructor.transformHtml.call(this, nodes);
	    (0, util.walkTree)(node, (item, next, parent) => {
	      var _parent$p;
	  ...
	  renderData(originData) {
	    ...
	    const layout = (0, d3Flextree.flextree)().children(d => {
	      var _d$p;
	    ...
	    const linkShape = d3$1.linkHorizontal();
	    const minX = d3$1.min(descendants, d => d.x - d.xSize / 2);
	    const maxX = d3$1.max(descendants, d => d.x + d.xSize / 2);
	    const minY = d3$1.min(descendants, d => d.y);
	    const maxY = d3$1.max(descendants, d => d.y + d.ySizeInner);
	    ...
	    const node = this.g.selectAll((0, util.childSelector)('g')).data(descendants, d => d.data.p.k);
	    ...
	    const rect = nodeMerge.selectAll((0, util.childSelector)('rect')).data(d => [d], d => d.data.p.k).join(enter => {
	    ...
	    const circle = nodeMerge.selectAll((0, util.childSelector)('circle')).data(d => d.data.c ? [d] : [], d => d.data.p.k).join(enter => {
	    ...
	    const foreignObject = nodeMerge.selectAll((0, util.childSelector)('foreignObject')).data(d => [d], d => d.data.p.k).join(enter => {
	    ...
	    const path = this.g.selectAll((0, util.childSelector)('path')).data(links, d => d.target.data.p.k).join(enter => {
	  ...
	  fit() {
	    ...
	    const initialZoom = d3$1.zoomIdentity.translate((offsetWidth - naturalWidth * scale) / 2 - minY * scale, (offsetHeight - naturalHeight * scale) / 2 - minX * scale).scale(scale);
	  ...
	  rescale(scale) {
	    ...
	    const transform = d3$1.zoomTransform(svgNode);
	...
	async function loadPlugins(items, options) {
	  ...
	  return (0, util.initializePlugins)(Markmap, items, options);
	}
	});

	var view$1 = /*@__PURE__*/unwrapExports(view);
