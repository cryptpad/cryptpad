define([
...
], function ($, Crypto, Curve, Hash, Listmap) {
    'use strict';
    var Msg = {
        inputs: [],
    };
    ...
    // createInvite(theirPublicKey, function (e, data) { /* data.url */ });
    window.createInvitation = Msg.createInvitation = function (common, theirPublic, cb) {

// theirPublic 'DHpRgRclYEvKdZO2gnN8bUyf6A4mEo8RmUGlGi9ChGo='

        var proxy = common.getProxy();
        var myPublic = proxy.curvePublic;

        var mySecret = proxy.curvePrivate;
        var keys = Curve.deriveKeys(theirPublic, mySecret);
        var encryptor = Curve.createEncryptor(keys);

        var channel = common.createChannelId();

    // create a channel encrypted with your curve private and their curvePublic
    // TODO support 'owners'. You should be able to delete the channel.
        var ListmapConfig = {
            data: {},
            network: common.getNetwork(),
            channel: channel,
            readOnly: false,
            validateKey: keys.validateKey,
            crypto: encryptor,
            username: 'invite-author',
            logLevel: 1,
        };

        var lm = Listmap.create(ListmapConfig);

        var inviteProxy = lm.proxy;
        inviteProxy.on('create', function () {
            console.log('created');
        })
        .on('ready', function () {
            /* do your stuff ...
                then, once everything is synchronized, abort */

            // a random channel
            var destinationChannel = common.createChannelId();

            // store all of the information necessary for them to become your friend
            var myData = createData(proxy, destinationChannel);
            Object.keys(myData).forEach(function (k) {
                inviteProxy[k] = myData[k];
            });

            //console.log(inviteProxy);
            var urlSafeChannel = common.hexToBase64(destinationChannel);
            var urlSafePublic = myPublic.replace(/\//g, '-');

            var res = {
                url: '/invite/#/1/' + urlSafeChannel + '/' + urlSafePublic + '/',
            };
...
/*
    * add their key to your 'friends' object as a pending request?
    * create a read-only link to that channel cb(void 0, { url: HERE })
      * /invite/#/1/invite/channel/yourCurvePublicKey
*/

/*  UI
    * copy link to clipboard
    * instruct user to send that link to the other user out-of-band
*/
