...
var applyPatch = function (realtime, author, patch) {
    if (author === realtime.userName && !patch.isInitialStatePatch) {
        ...
        // don't bother trying to JSON.parse things if authDoc is not defined
        // you'll never get past that point if you throw errors
        if (realtime.authDoc) {
            try {
                var applied = Patch.apply(realtime.uncommitted, realtime.authDoc);
                if (applied) {
                    JSON.parse(applied);
                } else {

                    console.error("[patchApplyError] Patch.apply(realtime.uncommitted, realtime.authDoc) // falsey");
                    console.log({
                        uncommitted: realtime.uncommitted,
                        authDoc: realtime.authdoc
                    });
                    //throw new Error("[patchApplyError] Patch.apply(realtime.uncommitted, realtime.authDoc) // falsey");
                }

            } catch (err) {
                console.error('[patchApplyError] realtime.uncommited => authDoc');
                console.error(err);
                console.log({
                    applied: applied,
                    uncommitted: realtime.uncommitted,
            ...
            try {
                ...
                console.error('[patchApplyError] realtime.authDoc');
                console.error(err);
                //throw new Error();
            }
        }
    ...
    var temp = Patch.apply(patch, realtime.authDoc);
    // changed by ansuz
    /*
    try {
        JSON.parse(temp);
